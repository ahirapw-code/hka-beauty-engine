...
// server/models/User.ts
var { Schema: Schema2, model, models } = mongoose2;
...
var cartItemSchema = z.object({
  ...
  quantity: z.number().int("quantity must be a positive integer.").positive("quantity must be a positive integer.").max(MAX_QUANTITY_PER_LINE, `quantity cannot exceed ${MAX_QUANTITY_PER_LINE}.`),
...
var attendanceRecordSchema = z.object({
...
var clockOutRecordSchema = z.object({
  clockOut: z.string().min(1, "clockOut is required."),
  notes: z.string().optional()
});
var clockInOutBodySchema = z.discriminatedUnion("action", [
  z.object({
...
var dataListQuerySchema = z.object({
  where: z.union([z.string(), z.array(z.string())]).optional().transform((v) => v === void 0 ? [] : Array.isArray(v) ? v : [v]).refine((clauses) => clauses.every((c) => whereClausePattern.test(c)), {
    message: 'Each "where" clause must look like "field,op,value" (op one of ==, !=, <, <=, >, >=).'
  }),
  orderBy: z.string().regex(/^[A-Za-z0-9_.]+(,(asc|desc))?$/, 'orderBy must look like "field" or "field,asc|desc".').optional(),
...
var bookingCreateSchema = z.object({
...
var batchSeedBodySchema = z.object({
  docs: z.array(
    z.object({
      id: z.string().min(1),
      data: z.record(z.any())
...
var { Schema: Schema3, model: model2, models: models2 } = mongoose3;
...
var { Schema: Schema4, model: model3, models: models3 } = mongoose4;
...
var { Schema: Schema5, model: model4, models: models4 } = mongoose5;
...
var { Schema: Schema6, model: model5, models: models5 } = mongoose6;
...
var { Schema: Schema7, model: model6, models: models6 } = mongoose7;
...
var { Schema: Schema8, model: model7, models: models7 } = mongoose8;
...
var { Schema: Schema9, model: model8, models: models8, Types } = mongoose9;
...
var { Schema: Schema10, model: model9, models: models9 } = mongoose10;
...
var { Schema: Schema11, model: model10, models: models10 } = mongoose11;
...
var { Schema: Schema12, model: model11, models: models11 } = mongoose12;
...
var { Schema: Schema13, model: model12, models: models12 } = mongoose13;
...
var { Schema: Schema14, model: model13, models: models13 } = mongoose14;
    ...
    try {
      ...
      while (true) {
        ...
        try {
          ...
          const txId = "TX-" + crypto2.randomUUID().split("-")[0].toUpperCase();
...
async function clockInOut(req, res) {
  try {
    ...
    if (action === "clockIn") {
      ...
      const attId = "att-" + Math.floor(Math.random() * 9e3 + 1e3) + "-" + crypto3.randomBytes(2).toString("hex");
...
async function uploadFile(req, res) {
  try {
    ...
    const id = crypto4.createHash("sha1").update(path).digest("hex");
...
async function connectToDatabase() {
  if (isConnected && mongoose16.connection.readyState === 1) {
    return mongoose16;
  }
  ...
  mongoose16.set("strictQuery", true);
  await mongoose16.connect(uri, {
    dbName: process.env.MONGODB_DB_NAME || void 0
  });
  isConnected = true;
  mongoose16.connection.on("error", (err) => {
    console.error("MongoDB connection error:", err);
  });
  mongoose16.connection.on("disconnected", () => {
    isConnected = false;
  });
