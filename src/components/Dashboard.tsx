...
interface DashboardProps {
  ...
  // Full staff/user directory - used to fold SALON_MANAGER accounts into
  // the sales share panel below alongside real Therapist records, the
  // same way POS.tsx surfaces them as assignable "therapists" at checkout.
  users: User[];
  products: Product[];
      ...
      <div id="branch-targets-section" className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm space-y-6">
        ...
        {targetBranch ? (
          ...
          (() => {
            ...
            // Combine real Therapist records with SALON_MANAGER users in
            // this branch - managers can be assigned as the servicing
            // staff at POS (see POS.tsx's activeTherapists merge), so their
            // sales need to show up here too, not just dedicated therapists.
            // A manager who already has a linked Therapist record (dual-
            // role staff) is skipped here to avoid counting them twice.
            const alreadyLinkedManagerIds = new Set(
              therapists.filter(t => t.linkedUserId).map(t => t.linkedUserId as string)
            );
            const managersAsStaff = users
              .filter(u =>
                u.role === 'SALON_MANAGER' &&
                (u.branch === targetBranch || u.branch === 'ALL') &&
            ...
            const allStaffForBranch: { id: string; name: string }[] = [
              ...therapists.filter(t => t.branch === targetBranch).map(t => ({ id: t.id, name: t.name })),
              ...managersAsStaff,
            ];

            // Filter current month transactions for staff in this branch
            const therapistsContribution = allStaffForBranch
              .map(staff => {
                const totalContributed = currentMonthTx
                  .filter(tx => tx.branch === targetBranch)
                  .reduce((sum, tx) => {
                    const staffServiceItems = tx.items.filter(item => item.type === 'service' && item.therapistId === staff.id);
                    return sum + staffServiceItems.reduce((itSum, item) => itSum + (item.price * item.quantity), 0);
                  }, 0);
                return {
                  therapist: staff,
            ...
            return (
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-5 space-y-4">
                  <div>
                    <h4 className="text-xs font-bold text-slate-700 uppercase tracking-wider font-mono">Staff Sales Share</h4>
                    <p className="text-[10px] text-slate-400">Services revenue generated by therapists & managers this month.</p>
                  </div>
